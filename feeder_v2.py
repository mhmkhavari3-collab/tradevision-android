...
# Phase 3.5: OANDA Stream + Price Cache + Candle Updater
import price_cache
from oanda_stream import OandaStreamManager
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # Phase 3.5: OANDA Pricing Stream + Candle Updater
        oanda_stream_task = None
        candle_updater_task = None
        ...
        if oanda_instruments and OANDA_TOKEN:
            ...
            oanda_stream_task = asyncio.create_task(oanda_stream.start(), name="OANDA-Stream")
            log.info(f"[OANDA-STREAM] Started for {len(oanda_instruments)} instruments")
            
            # Start Candle Updater (reads from Price Cache)
            candle_updater_task = asyncio.create_task(
                candle_updater.update_candles_from_cache(),
                name="Candle-Updater"
            )
            log.info("[CANDLE-UPDATER] Started")
        ...
        if oanda_stream_task:
            all_tasks.append(oanda_stream_task)
        if candle_updater_task:
            all_tasks.append(candle_updater_task)
